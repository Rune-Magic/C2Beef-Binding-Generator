...
static class CBindings
{
	...
	public class LibraryInfo
	{
		...
		public StringView beforeFirstDecl { get; set; } = null;
		public StringView afterLastDecl { get; set; } = null;
		...
		public delegate bool(char8* cursorFile, char8* currentHeader) includeCursorFromFile { get; set; } = null;
		public delegate bool(CXType paramType, StringView paramSpelling, StringView lastParamSpelling, StringView docs) isOutParam { get; set; } = null;
	}
	...
	public static void Generate(char8* header, StringView outputFile, StringView outputNamespace, LibraryInfo library, params Span<StringView> usingDepedencies)
	{
		...
		StreamWriter outputStream = outputFile.IsNull ?
			scope .(scope NullStream(), .UTF8, 0) :
			scope .()..Create(outputFile);

		String output = scope .(2048);
		...
		output.Append("""
			...
			namespace
			""", " ", outputNamespace, ";\n\n", library.beforeFirstDecl);

		bool newLine = false;
		String blockName = scope .(8);
		Clang.VisitChildren(Clang.GetTranslationUnitCursor(unit), (cursor, parent, data) =>
		{
			(String output, bool* newLine, char8* header, CXTranslationUnit unit, LibraryInfo library, String blockName, String lastDecls, StreamWriter outputStream) = *(.)data;

			Clang.GetSpellingLocation(Clang.GetCursorLocation(cursor), let file, ?, ?, ?);
			let filename = GetString!(Clang.GetFileName(file));
			if (filename == null) return .Continue;
			if (library.includeCursorFromFile == null)
			{
				if (!String.Equals(filename, header)) return .Continue;
			}
			else
			{
				if (!library.includeCursorFromFile(filename, header)) return .Continue;
			}
			...
			output.Clear();
			return .Continue;
		}, &(output, &newLine, header, unit, library, blockName, lastDecls, outputStream));

		EnsureBlockBase(blockName, "", output, &newLine);
		output.Append(library.afterLastDecl);
		outputStream.Write(output);
	}
	...
	public static bool IsStructOpaque(CXCursor cursor)
	{
		bool opaque = true;
	...
	{
		return (library.isOutParam != null && library.isOutParam(paramType, paramSpelling, lastParamSpelling, docs))
			|| (paramType.kind == .Pointer && Clang.GetPointeeType(paramType).kind != .Void && (
	...
	protected static void Type(CXType type, String output, LibraryInfo library, bool includeAttrsInAnon = false)
	{
		...
		case .LongDouble: output.Append("/*long double*/ System.Numerics.v128");
		case .Pointer:
	...
	protected static void Cursor(CXCursor cursor, String output, LibraryInfo library, CXTranslationUnit unit, StringView baseIndent, String blockName, String lastDecls, bool* newLine, bool inlinedType = false, bool includeAttrs = false, bool noEnsureBlock = true)
	{
			...
			Clang.VisitChildren(cursor, (cursor, parent, data) =>
			{
				...
				constExpr: do
				{
					Span<CXToken> tokens;
					{
						...
						tokens = .(tokenPtr, tokenCount);
						for (let token in tokens)
							if (Clang.GetTokenKind(token) == .Punctuation && String.Equals("=", GetString!(Clang.GetTokenSpelling(unit, token))))
							{
								tokens.RemoveFromStart(@token.Index+1);
								break;
							}
					}
		...
		case .TypedefDecl:
			...
			default:
				String typeStr = Type(type, ..scope .(64), library, true);
				if (Clang.Cursor_IsAnonymous(Clang.GetTypeDeclaration(type)) != 0)
				{
					typeStr..Replace("\n\t", "\n")..Insert(typeStr.IndexOf("\n{"), spelling);
					output.Append(indent, typeStr);
					if (hasDoc) output.Append('\n');
					*newLine = true;
					return;
				}
				...
				if (library.flags.HasFlag(.VulkanLike) && (typeStr == "VkFlags" || typeStr == "uint32") && lastDecls != null)
				{
					...
					if (lastDecls.Contains(str)) typeStr.Set(str[1...]);
				}
		...
		case .VarDecl:
			...
			case .External:
				output.Append(indent);
				String modifiedSpelling = scope .(64);
				bool modified = AddSpelling(cursor, library, modifiedSpelling, lastDecls);
				GetAttributes(cursor, library, .(GetString!(Clang.GetCursorSpelling(cursor))), type, output,
					StringView[?](modified ? scope String(64)..Append("LinkName(\"")..Append(GetString!(Clang.GetCursorSpelling(cursor)))..Append("\")") : "CLink"));
				output.Append(" public static extern ");
				Type(type, output, library); output.Append(' ');
				output.Append(modifiedSpelling, ";\n");
